...
def run_gsplat_ladder(package_dir: Path, out_dir: Path, gsplat_root: Path, steps: list[int] | None = None, qa_summary_dir: Path | None = None, image_dir: str = "images", sparse_dir: str = "sparse/0", strategy: str = "mcmc", data_factor: int = 1, dry_run: bool = False, sanitize_non_finite_ply: bool = False, max_psnr_drop: float = 0.5, max_ssim_drop: float = 0.02, max_mae_increase: float = 0.01, max_correlation_drop: float = 0.03, photometric: str | None = None, masks: str = "auto", normalization: str = "auto", mcmc_refine_every: str | int = "auto", depth_supervision: str = "auto", normal_supervision: str = "auto") -> dict[str, Any]:
    ...
    for step in step_values:
        ...
        try:
            run_summary = run_gsplat(package_dir, rung_dir, gsplat_root, steps=step, strategy=strategy, image_dir=image_dir, sparse_dir=sparse_dir, data_factor=data_factor, dry_run=dry_run, photometric=photometric, masks=masks, normalization=normalization, mcmc_refine_every=mcmc_refine_every, depth_supervision=depth_supervision, normal_supervision=normal_supervision)
...
def main(argv: list[str] | None = None) -> None:
    ...
    summary = run_gsplat_ladder(args.package, args.out, args.gsplat_root, steps=parse_steps(args.steps), qa_summary_dir=args.qa_summary_dir, image_dir=args.image_dir, sparse_dir=args.sparse_dir, strategy=args.strategy, data_factor=args.data_factor, dry_run=args.dry_run, sanitize_non_finite_ply=args.sanitize_non_finite_ply, max_psnr_drop=args.max_psnr_drop, max_ssim_drop=args.max_ssim_drop, max_mae_increase=args.max_mae_increase, max_correlation_drop=args.max_correlation_drop, photometric=args.photometric, masks=args.masks, normalization=args.normalization, mcmc_refine_every=args.mcmc_refine_every, depth_supervision=args.depth_supervision, normal_supervision=args.normal_supervision)
